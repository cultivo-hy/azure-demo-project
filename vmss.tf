resource "azurerm_virtual_machine_scale_set" "user16vmss" {
name = "user16vmss"
...
os_profile {
    ...
    admin_username = "hystar"  ## VM 에 접속할 계정
...
ssh_keys {
    path = "/home/hystar/.ssh/authorized_keys"   ## pwd 실행후 경로설정 ex) /home/user16 등 
...
network_profile {
        ...
        primary = true
        subnet_id = azurerm_subnet.user16-subnet1.id
        load_balancer_backend_address_pool_ids = [azurerm_lb_backend_address_pool.user16-bpepool.id]
        load_balancer_inbound_nat_rules_ids = [azurerm_lb_nat_pool.lbnatpool.id]
    }
        network_security_group_id = azurerm_network_security_group.user16nsg.id
}
tags = {
    environment = "staging"
    }
